...
- 重构为多工人并行队列：每个 ComfyUI 节点对应一个工人，多节点并行处理
- 新增 `_processing_count` 精确追踪队列位置（含正在执行的任务）
- 新增独立 Tagger 队列，`/tagger` 图片反推标签不阻塞绘图
- 新增 `/tagger` 命令：调用 WD14 Tagger 反推图片标签，支持直接发图和回复引用图片
- 新增服务器 Token 鉴权支持：`servers` 配置支持 `id;IP:端口;token` 格式，携带 `Authorization: Bearer` 头
